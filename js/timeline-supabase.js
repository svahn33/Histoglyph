...
const collectionContainer = document.querySelector("#timeline-collection-groups");
const settingsDialog = document.querySelector("#timeline-settings-dialog");
const settingsForm = document.querySelector("#timeline-settings-form");
const selectedCollectionInput = document.querySelector("#timeline-selected-collection-id");
const settingsTitle = document.querySelector("#timeline-settings-title");
const peopleInput = document.querySelector("#timeline-setting-people");
const includeAllInput = document.querySelector("#timeline-setting-include-all");
const difficultyInput = document.querySelector("#timeline-setting-difficulty");
const birthYearMinInput = document.querySelector("#timeline-setting-birth-year-min");
const birthYearMaxInput = document.querySelector("#timeline-setting-birth-year-max");
const birthYearMinEntry = document.querySelector("#timeline-birth-year-min-entry");
const birthYearMaxEntry = document.querySelector("#timeline-birth-year-max-entry");
const birthYearDualRange = document.querySelector("#timeline-birth-year-dual-range");
const birthYearRangeTrack = birthYearDualRange?.querySelector(".birth-year-range-track");
const birthYearMinThumb = document.querySelector("#timeline-birth-year-min-thumb");
const birthYearMaxThumb = document.querySelector("#timeline-birth-year-max-thumb");
const settingsNote = document.querySelector("#timeline-settings-note");
const startSettingsButton = document.querySelector("#timeline-start-settings");
const closeSettingsButton = document.querySelector("#timeline-close-settings");
const cancelSettingsButton = document.querySelector("#timeline-cancel-settings");
...
async function loadBirthYearBounds(collection) {
  ...
  if (!bounds) {
    const { data, error } = await supabase.rpc("get_timeline_birth_year_bounds", {
      p_collection_slug: requestedSlug
    });
    if (error) {
      console.error(error);
      settingsNote.textContent = "Birth-year filtering is unavailable. Run the V50 Timeline database migration first.";
      return;
    }
...
function openGameSettings(collection) {
  ...
  peopleInput.value = String(Math.max(2, collection.default_rounds || 10));
...
function makeCollectionCard(collection) {
  ...
  if (collection.status === "available" && Number(collection.available_people) >= 2) {
...
function peopleCount() {
  ...
  return Number.isFinite(value) ? Math.max(2, Math.min(100, value)) : 10;
}
...
function updateSettingsNote() {
  ...
  const rangeLabel = range
    ...
    : "birth years loading";
  settingsNote.textContent = `Current settings: ${countLabel} · ${difficultyLabel} · ${rangeLabel}.`;
}
...
settingsForm.addEventListener("submit", event => {
  ...
  location.href = `timeline-play.html?${params}`;
});
...
const { data, error } = await supabase.rpc("list_timeline_collections");
if (error) {
  console.error(error);
  const fallback = FALLBACK_COLLECTIONS.map(collection => ({
    ...
    default_rounds: Math.max(2, collection.roundLimit || 10),
    available_people: 0
  }));
  renderCollections(fallback);
  const warning = document.createElement("p");
  warning.className = "data-warning";
  warning.textContent = `Timeline is not connected to the database yet: ${error.message}`;
